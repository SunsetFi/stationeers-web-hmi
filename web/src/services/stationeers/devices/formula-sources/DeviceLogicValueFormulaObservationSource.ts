  ...
  implements FormulaObservationSource
{
  ...
  readonly name = "deviceLogicValue";

  resolve(args: Observable<any>[]): Observable<any> {
    const [deviceId, logicValue, defaultValue] = args;
    if (!deviceId) {
    ...
    const resolvedDefaultValue = defaultValue ?? observableOf(Number.NaN);
    return combineLatest([data$, logicValue, resolvedDefaultValue]).pipe(
      map(
        ([data, logicValue, defaultValue]) =>
          data.logicValues[String(logicValue)] ?? defaultValue
      ),
      distinctUntilChanged()
    );
  }
